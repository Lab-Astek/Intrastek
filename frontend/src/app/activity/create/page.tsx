...
export default function Home() {
  ...
  function handleSubmit() {
    if (startDate === undefined || endDate === undefined)
      return;
    createActivity(name, asteks, activities[activitiyTypeIdx], modules[moduleIdx], { start: startDate, end: endDate })
  }
    ...
    <div className="flex min-h-screen flex-col items-center justify-between p-24">
      <form noValidate autoComplete="off">
        <Stack spacing={3}>
          ...
          >
            <TextField label="Name" margin="dense" focused onChange={handleChangeName} />
            <TextField defaultValue={0} type="number" label="Asteks" margin="dense" focused onChange={handleChangeAsteks} />
          </Box>
          <ActivityPicker />
          <ModulePicker />
          <DatePicker label="Start" onChange={(value, ctx) => {
            if (value) {
              setStartDate(value.toDate())
            }
          }} />
          <DatePicker label="End" onChange={(value, ctx) => {
            if (value) {
